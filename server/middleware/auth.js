...
// This function verifies if the user id passed in the request params and user id present in jwt token is same
// If not same restrict access
export const verifyIsSameUser = async (req, res, next) => {
    try{
        verifyToken(req, res, () => {
            if(req.params.userId === req.user._id) {
                next();
            } else {
                next(createError(403, 'Unauthorized Profile Access'))
            }
        });
    } catch (err) {
